...
```bash
bash "$S" update --all           # plan every installed persona; writes nothing
bash "$S" update --all --yes     # apply; NEVER add --force here
```

The script prints **exactly one `TRUECAST_RESULT` line per persona** — a member that cannot be updated
gets `status=failed … exit=<code> reason=<token>` rather than silence. A failure on one member never
stops the others, and the run exits **6** if any member failed.

- Report **one line per persona**: updated / already up to date / failed with its reason. Do not
  summarise as "all up to date" unless every line says `up-to-date`. The last line of stdout is one
  member's result, not a verdict on the run — use the exit code for that.
- Ask once before applying, listing every persona that would change.
- `reason=drift` on a member: that persona's agent file was generated by truecast and differs. Do **not**
  re-run `--all --force`; that would blanket-discard edits across every persona. Follow up on that one:
  `bash "$S" update <name>` to show its diff, then `bash "$S" update <name> --force --yes` if the user
  approves it specifically.
- `reason=foreign` on a member: `~/.claude/agents/<name>.md` exists and truecast did not write it.
  Tell the user to rename or delete that file. `--force` will not override it.
- `reason=busy`: another truecast operation held that persona's lock. Retry that one persona shortly.
