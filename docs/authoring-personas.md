...
`personas/<name>/subagent.md` is likewise generated by `truecast publish` (it's the pre-rendered
subagent template `/truecast:install` copies, with `{{TRUECAST_HOME}}` substituted at install time).
Never hand-edit it — regenerate with `truecast publish`; `--check` fails CI if it drifts.
