...
#
# Exit codes: 0 ok (incl. status=plan) · 2 usage · 3 not-found · 4 busy · 5 foreign · 7 precondition · 8 io
#
...
plan_report() {
  ...
  if [ -e "$meta" ] && ! grep -q "#personas/$name\"" "$meta" 2>/dev/null; then
    say "  source-mismatch: this persona was installed from $(sed -n 's/.*"source"[[:space:]]*:[[:space:]]*"\([^"]*\)".*/\1/p' "$meta" | head -n 1)"
...
cmd_update() {
  if [ "$ALL" = 1 ]; then
    ...
    for name in $(installed_personas); do
      ...
      bash "$SCRIPT" update "$name" $flags || say "  (continuing with $name failed — moving on)"
    done
    if [ "$any" = 0 ]; then
